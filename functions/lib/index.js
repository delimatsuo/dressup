...
async function processLegacySinglePose(params) {
    const { userImageUrl, garmentId, sessionId, garmentImageUrl, poseType, instructions, startTime } = params;
    // Validate legacy inputs
    if (!userImageUrl || (!garmentId && !garmentImageUrl)) {
        throw new https_1.HttpsError('invalid-argument', 'Legacy mode requires userImageUrl and either garmentId or garmentImageUrl');
    }
